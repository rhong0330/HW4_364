...
# DONE 364: Check out the included file giphy_api_key.py and follow the instructions in TODOs there before proceeding to view functions.

# OKAY 364: All templates you need are provided and should not be edited. However, you will need to inspect the templates that exist in order to make sure you send them the right data!
...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/hongjisuHW4db" # DONE 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
# DONE 364: Set up association Table between search terms and GIFs (you can call it anything you want, we suggest 'tags' or 'search_gifs').
...
# DONE 364: Set up association Table between GIFs and collections prepared by user (you can call it anything you want. We suggest: user_collection)

user_collection = db.Table('user_collection', db.Column('user_id', db.Integer,db.ForeignKey('gif.id')),db.Column('personalgifcollection_id',db.Integer,db.ForeignKey('personalgifcollection.id')))
...
class Gif(db.Model):
    ...
    # Replace with code
    # DONE 364: Add code for the Gif model such that it has the following fields:
    # id (Integer, primary key)
    # title (String up to 128 characters)
    # embedURL (String up to 256 characters)

    # DONE 364: Define a __repr__ method for the Gif model that shows the title and the URL of the gif
    def __repr__(self):
...
class PersonalGifCollection(db.Model):
    ...
    # DONE 364: Add code for the PersonalGifCollection model such that it has the following fields:
    # id (Integer, primary key)
...
class SearchTerm(db.Model):
    ...
    # DONE 364: Add code for the SearchTerm model such that it has the following fields:
    # id (Integer, primary key)
    ...
    # DONE 364: Define a __repr__ method for this model class that returns the term string
    def __repr__(self):
...
# DONE 364: The following forms for searching for gifs and creating collections are provided and should not be edited. You SHOULD examine them so you understand what data they pass along and can investigate as you build your view functions in TODOs below.
...
def get_gifs_from_giphy(search_string):
    ...
    return result['data']
    # Replace with code
    # DONE 364: This function should make a request to the Giphy API using the input search_string, and your api_key (imported at the top of this file)
...
def get_or_create_gif(title, url):
    ...
    if g:
        ...
        return g
    # Replace with code
    # DONE 364: This function should get or create a Gif instance. Determining whether the gif already exists in the database should be based on the gif's title.
...
def get_or_create_search_term(term):
    ...
    if search_term:
        ...
        db.session.commit()
        return search_term
    # DONE 364: This function should return the search term instance if it already exists.
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    # DONE 364: This function should get or create a personal gif collection. Uniqueness of the gif collection should be determined by the name of the collection and the id of the logged in user.
...
def index():
    ...
    # DONE 364: Edit this view function, which has a provided return statement, so that the GifSearchForm can be rendered.
